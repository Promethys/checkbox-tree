...
```bash
php artisan vendor:publish --tag="checkbox-tree-migrations"
php artisan migrate
```
...
```bash
php artisan vendor:publish --tag="checkbox-tree-config"
```

Optionally, you can publish the views using

```bash
php artisan vendor:publish --tag="checkbox-tree-views"
```
...
```php
$checkboxTree = new Promethys\CheckboxTree();
echo $checkboxTree->echoPhrase('Hello, Promethys!');
```
